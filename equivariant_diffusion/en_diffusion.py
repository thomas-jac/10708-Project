...
# def uniform_schedule(timesteps, s=0.008, raise_to_power: float = 1):
#    """
#     A uniform noise schedule that distributes noise levels evenly.
#     Args:
#         timesteps (int): The total number of timesteps.
#         s (float): A small value to ensure numerical stability and precision.
#     Returns:
#         np.ndarray: An array of cumulative noise levels (alphas^2).
#     """
...
def uniform_schedule(timesteps: int, s=1e-4):
    """
    A uniform noise schedule that distributes noise levels evenly.
    ...
    """
    steps = timesteps + 1
    x = np.linspace(0, 1, steps)  # Uniformly spaced values between 0 and 1

    # Generate uniform schedule
    alphas2 = 1 - x

    # Clip the values to ensure numerical stability
    alphas2 = np.clip(alphas2, s, 1 - s)

    return alphas2


def constant_schedule(timesteps: int, s=0.9):
    """
    A constant noise schedule defined by s
    """
    steps = timesteps + 1
    x = np.full(steps, s)
    alphas2 = 1 - x
    print("Alphas2 from constant noise: ", alphas2)
...
class PredefinedNoiseSchedule(torch.nn.Module):
    ...
    def __init__(self, noise_schedule, timesteps, precision):
        ...
        elif noise_schedule == 'uniform':
            alphas2 = uniform_schedule(timesteps, s=precision)
        elif noise_schedule == 'constant':
            alphas2 = constant_schedule(timesteps, s=precision)
        else:
...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    @torch.no_grad()
    def sample_using_z(self, z, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False):
        """
        Draw samples from the generative model.
        """
        # if fix_noise:
        ...
        # sigma_val = self.sigma(self.gamma())
...
class EnLatentDiffusion(EnVariationalDiffusion):
    ...
    def score(self, y, n_samples, n_nodes, node_mask, edge_mask, context, keep_frames=None, fix_noise=False):
        ...
        """
        xhat = super().sample_using_z(y, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise)
        zeros = torch.zeros((y.size(0), 1), device=y.device)
        gamma_0 = self.gamma(zeros)
        # print('gamma_0:', gamma_0)
        return (xhat - y) / (gamma_0[0] ** 2)
    
    # We will write a function to perform a random walk in the latent space of the DM
    @torch.no_grad()
    def wjs_walk_steps(self, y, v, n_samples, n_nodes, node_mask, edge_mask, context, n_steps = 1000, delta = 0.5, friction = 1.0, lipschitz = 1.0):
        ...
        for step in range(n_steps):
            y += delta * v / 2  # y_{t+1}
            psi = self.score(y, n_samples, n_nodes, node_mask, edge_mask, context, keep_frames=None, fix_noise=False)
            v += u * delta * psi / 2  # v_{t+1}
            v = zeta1 * v + u * delta * psi / 2 + math.sqrt(u * (1 - zeta2)) * torch.randn_like(y)  # v_{t+1}
            y += delta * v / 2  # y_{t+1}
            print('Shape at step {}'.format(step), y.shape)
            # print('Error val:', ( * (1 - node_mask)).abs().max().item())
        # torch.cuda.empty_cache()
        # print()
        return y, v

    @torch.no_grad()
    def infer_sample(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False):
        """
        Draw samples from the generative model.
        """
        if fix_noise:
        ...
        # diffusion_utils.assert_mean_zero_with_mask(z[:, :, :self.n_dims], node_mask)
        print('Before walk:', z.shape)
        v = 0 * torch.randn_like(z)
        z, v = self.wjs_walk_steps(z, v, n_samples, n_nodes, node_mask, edge_mask, context, n_steps=70, delta=0.5, friction=1.0, lipschitz=1.0)

        # Finally sample p(x, h | z_0).
        z_x, z_h = self.sample_p_xh_given_z0(z, node_mask, edge_mask, context, fix_noise=fix_noise)
