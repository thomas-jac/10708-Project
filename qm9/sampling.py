...
def sample_chain(args, device, flow, n_tries, dataset_info, prop_dist=None):
    ...
    if args.probabilistic_model == 'diffusion':
        ...
        for i in range(n_tries):
            chain = flow.sample_chain(n_samples, n_nodes, node_mask, edge_mask, context, keep_frames=1)
...
def sample(args, device, generative_model, dataset_info,
    ...
    if args.probabilistic_model == 'diffusion':
        ...
        x, h = generative_model.infer_sample(batch_size, max_n_nodes, node_mask, edge_mask, context, fix_noise=fix_noise)
